...
La mémoire est accessible en lecture et écriture mais elle ne peut être adressée depuis le CPU.
...
L'exemple suivant permet de calculer le produit scalaire de deux vecteurs.

Lien sur <<{sourcesdir}/dot_cpu.cu,dot_cpu.cu>>.
[source,C]
include::{sourcesdir}/dot_cpu.cu[]

La mise en oeuvre parallèle sur GPU passe par les différentes étapes mentionnées précédemment :

* Allocation mémoire sur GPU
* Initialisation de celle-ci
* Lancement des threads
* Recopie du résultat
* et achèvement du calcul

Lien sur <<{sourcesdir}/dot.cu,dot.cu>>.
[source,C]
include::{sourcesdir}/dot.cu[]

Cet exemple utilise une zone de mémoire Shared pour permettre aux threads au sein du même block, de calculer une partie du produit scalaire (celle correspondant au traitement des threads au sein du block).

La mise en oeuvre d'une réduction (au sein du block) est proposée.
...
On prend un texte en entrée et on veut calculer le nombre d’occurrences de chaque lettre.
